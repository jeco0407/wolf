...
- AI 發言節奏：拿到 LLM 內容就先顯示並朗讀，再依字數停留（`readMs`：每字 0.23 秒加 1 秒，3–40 秒，配合中文語音約每秒 4–5 字）才換下一位。理由：念完、聽完才換人（使用者反映只看文字來不及），也讓呼叫頻率壓在 Groq 每分鐘 token 額度內。
...
- 房主按「開始」→ 空位依座位補上 AI（隨機抽人設）→ 分配身份 → 進入第 1 夜。發牌時避免真人連續抽到同一個角色：房間記錄每位真人上一局的角色（`rooms.lastRoles`），重複時重新發牌最多 6 次、取重複最少的一次；單機練習用戰績紀錄的上一局角色做同樣的事。發牌本身是公平的（模擬 12,000 次，每人抽到狼人約 33%）。遊戲開始後不能再加入（可以之後做觀戰）。
...
- **AI 發言朗讀**：用 speechSynthesis 依人設調整聲音（每位 AI 的 pitch 不同、語速 0.8–1.0，法官 0.85；使用者反映原本 0.95–1.25 太快），設定頁可關閉或調整「朗讀速度」倍率（`ttsRate`，0.8×–1.2×）。本機模擬由 `LocalGame` 的 Narrator 朗讀並等念完；連線對局由 `GameScreen`（`readAiSpeech`）在每位玩家的裝置各自朗讀，伺服器用 `readMs` 預留朗讀時間。AI 不加入 LiveKit 頻道。
