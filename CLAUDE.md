...
- **AI 發言朗讀**：用 speechSynthesis 依人設調整聲音（每位 AI 的 pitch／rate 不同），可在設定中關閉。本機模擬由 `LocalGame` 的 Narrator 朗讀並等念完；連線對局由 `GameScreen`（`readAiSpeech`）在每位玩家的裝置各自朗讀，伺服器用 `readMs` 預留朗讀時間。AI 不加入 LiveKit 頻道。
